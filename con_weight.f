     ...
     1  ,endtime,holes)
c        program cont_weight 
C       Propogate my walkers to get continuous weighting factors
...
C       Find total weight
        ...
        tot_run = endtime/dt
        alp = 0.5/dt
        call calc_psi(n,r_old,sig_bend,nmono,psi_old)
        call calc_grad(n,r_old,sig_bend,psi_old,nmono,q_f_rold,first,
     1  second)
        do i = 1,n
     ...
     1      nmono,drdx(:,:,i),dr2dx2(:,:,i))
        ...
        do while (time.lt.endtime)
            ...
            call calc_psi(n,r_new,sig_bend,nmono,psi_new)
            call calc_grad(n,r_new,sig_bend,psi_new,nmono,q_f_rnew,
...
c     1      dfloat(n)
        ...
        do i = 1,n
            weight_tot(n_coord(i)) = weight_tot(n_coord(i))+
     1      con_weight(n_coord(i))
        enddo
        return
        end subroutine
