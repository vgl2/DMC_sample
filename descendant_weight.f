...
C       wavefunction. Victor Lee 7/12/2016
     ...
     1  standard_box(nbin),old_box(nbin),analytical_soln(nbin),fam(nmax)
     1  ,analytical_squared(nbin),analytical_box(nbin),dr(nmax)
        open(unit=9,file='descendant.dat',status='old')
        open(unit=10,file='plot_descendant.dat',status='unknown')
        omega = 4401.213d0/219474.6
     ...
     1  z1(i))**2)
            ...
            zero = nzero
            descend = ndescend
            print *, zero/descend,'ratio of zeros'
            rmin = 0.
            rmax = 3.
            do j = 1,nbin
                old_box(j) = box(j)
            enddo
            if (k.gt.1) then
                call hist_weight(r,nbin,n,box,rval,rmin,rmax,fam,
     1          descend)
                do i = 1,nbin
